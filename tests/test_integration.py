...
def test_crystallize_excludes_skill_kind(mem):
    ...
    mem.crystallize("部署")  # 生成 skill 1
    res = mem.crystallize("部署")  # 再次结晶：skill 不算素材
    assert res["ok"] and res["n_sources"] == 2
...
def test_panel_commands_e2e(cluster):
    ...
    with httpx.Client(base_url=f"http://127.0.0.1:{port}", timeout=15) as c:
        ...
        assert "已结晶" in r and "条经验" in r
        r = c.post("/api/say", json={"text": "skills"}).json()["reply"]
        assert "技能库" in r and "免费模型故障处理" in r
        # tasks 面板
